...
function fetchLatestHubble() {
  return fetch("http://35.192.94.107:3333/api/nasa/daily-hubble", {
    mode: "cors",
    cache: "default",
  });
}
...
function fetchDailyNASA() {
  return fetch("http://35.192.94.107:3333/api/nasa/daily-nasa", {
    mode: "cors",
    cache: "default",
  });
}
...
function latestHubble(data) {
  const url = data.image_files[0].file_url;
  return {
    type: GET_LATEST_HUBBLE,
    latestHubble: {
      title: data.name,
      image: url,
      hubbleText: data.description,
